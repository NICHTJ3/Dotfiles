...
function M.build(cb)
  M.ensure_treesitter_cli(function(_, err)
    ...
    if ok then
      ...
      Core.error(lines, { title = 'Core Treesitter' })
    end
  end)
end
...
function M.ensure_treesitter_cli(cb)
  ...
  mr.refresh(function()
    ...
    if not p:is_installed() then
      Core.info 'Installing `tree-sitter-cli` with `mason.nvim`...'
      p:install(
        nil,
        vim.schedule_wrap(function(success)
          if success then
            Core.info 'Installed `tree-sitter-cli` with `mason.nvim`.'
            cb(true)
          else
